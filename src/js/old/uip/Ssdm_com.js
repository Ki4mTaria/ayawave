...
function Fsdm_init( tgt_id ) {
  try {
    ...
    if( false == Fcom_isLoaded( DINI_LOAD_VELOCITY ) ) {
      Fcom_startLoad( DINI_LOAD_VELOCITY );
      Fcom_loadJs(
        ["./load/js/velocity"] ,
        function() {
          try {
            ...
            Fcom_endLoad( DINI_LOAD_VELOCITY );
